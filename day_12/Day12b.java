...
class Day12b {
    ...
    public boolean isVisitableAgain(ArrayList<String> path, String newNode) {
System.out.println( "isVisitableAgain path="+path+ " newNode="+newNode);

        HashMap<String, Integer> counts = new HashMap<String, Integer>();
        for (String node : path) {
            if (isLowercase(node)) {
                if (!counts.containsKey(node))
                    counts.put(node, Integer.valueOf(1));
                else
                    counts.put(node, Integer.valueOf(counts.get(node).intValue() + 1));
            }
        }

        if (!counts.containsKey(newNode)) {
            System.out.println("true");
            return true;
        }

        counts.put(newNode, Integer.valueOf(counts.get(newNode).intValue() + 1));

        int countsOf2 = 0;
        for (String node : counts.keySet()) {
            if (counts.get(node).intValue() > 2) {
                System.out.println("true");
                return false;
            }
            if (counts.get(node).intValue() > 1)
                countsOf2++;
        }
        System.out.println( countsOf2 <= 1 );
        return countsOf2 <= 1;
    }
    ...
    public void searchPathRec(ArrayList<String> path, String newNode) {
        ...
        if (!"start".equals(newNode) && isLowercase(newNode) && !isVisitableAgain(path, newNode))
            return;
